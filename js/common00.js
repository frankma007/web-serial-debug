;(function () {
	...
	function stringReverse(hexString) {
	
		let parts = hexString.split(' '); // 分割字符串为数组 ['c8', '81', '88']
		parts.reverse(); // 反转数组 ['88', '81', 'c8']
		let newHexString = parts.join(' '); // 重新组合为字符串 '88 81 c8'
...
const  qz ='EB 90 D2 21 27 FF 2C 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 64 64'
	...
	// 添加 serial-params-wxjd 的 change 事件监听
	const wxjdSwitch = document.getElementById('serial-params-wxjd');
	// const sendContent = document.getElementById('serial-send-content');

	if (wxjdSwitch && sendContent) {
	    wxjdSwitch.addEventListener('change', function () {
